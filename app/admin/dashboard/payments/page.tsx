...
export default function AdminPage () {
    ...
    const [sortBy, setSortBy] = useState<keyof PaymentInfo>("created_at");
    ...
    const filteredAndSorted = useMemo(() => { //lưu kết quả xử lý từ dữ liệu API, không cần gọi lại
        ...
        if (search.trim()) {
            ...
            result = result.filter(des =>
                des.booking_code.toLowerCase().includes(term) 
            );
        }
    ...
    //sort
    const handleSort = (field: keyof PaymentInfo) => {
        if (sortBy === field) {
    ...
    return (
        <div className="py-2 max-w-7xl mx-auto h-screen relative">
            <div className="w-full max-w-[96%] mx-auto">
                {/* title */}
                <h1 className="text-3xl font-bold mb-2 text-main">Quản lý thanh toán</h1>
                <div className="flex gap-4 items-center">
                    ...
                    <button 
                        ...
                        onClick={() => router.push("/admin/dashboard/destinations/create")}
                    >
                        ...
                        <table className="w-full min-w-max">
                                ...
                                <tr>
                                    ...
                                    >
                                        <div className="flex items-center gap-2">
                                            ...
                                            <SortIcon isActive={sortBy === "payment_method"} direction={sortBy === "payment_method" ? sortOrder : null} />
                                        </div>
                                    </th>
                                    <th 
                                    ...
                                    >
                                        <div className="flex items-center gap-2">
                                            Giá tiền
                                            <SortIcon isActive={sortBy === "amount"} direction={sortBy === "amount" ? sortOrder : null} />
                                        </div>
                                    </th>
                                    <th 
                            ...
                            <tbody className="divide-y">
                                {paginated.map(payment => (
                                    <tr key={payment.payment_id} className="hover:bg-gray-50 transition items-center">
                                        ...
                                        <td className="px-6 py-4 font-medium max-w-[180px] overflow-hidden">
                                            <a
                                                className="cursor-pointer hover:text-blue-600 block truncate whitespace-nowrap"

...
export default function PaymentPage() {
    ...
    const calculatePaymentAmount = useCallback((): number => {
        ...
        // Nếu đã đặt cọc (DEPOSIT), chỉ thanh toán phần còn lại (60%)
        if (bookingData.status === 'DEPOSIT') {
            return bookingData.remaining_amount || 0;
        }

        // Nếu chưa thanh toán gì (PENDING)
        // User có thể chọn: Thanh toán 100% hoặc Đặt cọc 40%
        if (paymentType === "FULL") {
    ...
    // Check if booking is already deposited
    const isDepositStatus = booking?.booking.status === 'DEPOSIT';

    // Disable payment type selection if already deposited
    const canSelectPaymentType = !isDepositStatus && booking?.booking.status === 'PENDING';
    ...
    const handleConfirmPayment = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (isDepositStatus || paymentType === "FULL") {
                    router.push('/account/booking?status=paid');
                } else {
                    router.push('/account/booking?status=deposit');
                }
            } else {
    ...
    return (
        <div className="w-8/10 mx-auto flex flex-col gap-4 py-6">
            {/* Payment Type Selection - Only show if booking is PENDING and not deposited */}
            {canSelectPaymentType && (
            ...
            {/* If already deposited, show info */}
            {isDepositStatus && (
                <div className="border-2 border-yellow-500 bg-yellow-50 rounded-lg p-6">
                    ...
                    <div className="space-y-2 text-gray-700">
                        <p>Bạn đã đặt cọc: <span className="font-semibold">{formatPrice(booking.booking.paid_money)}</span></p>
                        <p className="text-xl font-bold text-yellow-800">
                            Số tiền cần thanh toán: {formatPrice(booking.booking.remaining_amount || 0)}
                        </p>
                    </div>
                </div>
            )}

...
export default function RemainingPaymentsPage() {
    ...
    const fetchBooking = async () => {
        try {
            ...
            if (bookingData.is_full_payment) {
                setError("Booking này đã thanh toán đủ")
                return
            }
...
}, [booking_id, isLoggedIn, authLoading])
    ...
    useEffect(() => {
        const fetchQRCode = async () => {
            ...
            try {
                const url = `${API_URL}/payment/generate-qr/${booking_id}?amount=${amount}`;
    ...
    const getPaymentMethodValue = (method: PaymentMethod) => {
        if (method === "QR") return "bank_transfer";
    ...
    const handleConfirmPayment = async () => {
        ...
        formData.append("amount", booking.remaining_amount.toString());
        formData.append("images", imageFile);

        try {
            const res = await fetch(`${API_URL}/payment/create-remaining`, {
                method: "POST",
                credentials: "include",
                body: formData,
            });
            ...
            if (res.ok) {
                alert(result.message || "Gửi thanh toán thành công");

                setSubmitSuccess(true);

                setTimeout(() => {
                    router.replace("/account/booking?status=confirmed"); 
                }, 800);
    ...
    if (authLoading || bookingLoading) {
        return <div className="w-8/10 mx-auto text-center py-8">Đang tải dữ liệu...</div>
    }
